...
class Program {
	...
	public void display(){
		for(Declaration each_item : decpart)
			System.out.print(each_item.t.toString() + " " + each_item.v.toString() + " ");
		System.out.println();
		int indentCount = 0;
		displayStatement(body, indentCount);
	}

	private void displayStatement(Statement b, int indentCount){
		if(b instanceof Block){
			Block block_b = (Block)b;
			displayBlock(block_b, indentCount);
		}else{
			if(b instanceof Assignment){
				Assignment a = (Assignment)b;
				System.out.println("=");
				System.out.println("\t" + a.target.toString());
				displayExpression(a.source, indentCount+1);
			}else if(b instanceof Conditional){
				Conditional c = (Conditional)b;
				System.out.println("If");
				displayExpression(c.test, indentCount+1);
				displayStatement(c.thenbranch, indentCount+1);
				if(!(c.elsebranch instanceof Skip))
					displayStatement(c.elsebranch, indentCount+1);
			}else if(b instanceof Loop){
				Loop l = (Loop)b;
				System.out.println("While");
				displayExpression(l.test, indentCount+1);
				displayStatement(l.body, indentCount+1);
			}else if(b instanceof Skip){
	...
	private void displayBlock(Block b, int indentCount){
		for(Statement each_item : b.members){
			if(each_item instanceof Block){
				displayBlock((Block)each_item, indentCount+1);
			}else if(each_item instanceof Assignment){
				Assignment a = (Assignment)each_item;
				System.out.println("=");
				System.out.println("\t" + a.target.toString());
				displayExpression(a.source, indentCount+1);
			}else if(each_item instanceof Conditional){
				Conditional c = (Conditional)each_item;
				System.out.println("If");
				displayExpression(c.test, indentCount+1);
				displayStatement(c.thenbranch, indentCount+1);
				if(!(c.elsebranch instanceof Skip))
					displayStatement(c.elsebranch, indentCount+1);
			}else if(each_item instanceof Loop){
				Loop l = (Loop)each_item;
				System.out.println("While");
				displayExpression(l.test, indentCount+1);
				displayStatement(l.body, indentCount+1);
			}else if(each_item instanceof Skip){
	...
	private void displayExpression(Expression e, int indentCount){
		String indent = "";
		for(int i=0; i < indentCount ; i++)
			indent = indent + "\t";
		if(e instanceof Binary){
			Binary bi = (Binary)e;
			System.out.println(indent + bi.op.toString());
			displayExpression(bi.term1, indentCount + 1);
			displayExpression(bi.term2, indentCount + 1);
		}else if(e instanceof Unary){
			Unary u = (Unary)e;
			System.out.println(indent + u.op.toString());
			displayExpression(u.term, indentCount + 1);
		}else{
