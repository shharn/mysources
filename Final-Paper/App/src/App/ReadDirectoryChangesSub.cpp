...
namespace ReadDirectoryChangesPrivate
{
	...
	BOOL CReadChangesRequest::IsVideoFile(CStringW file)
	{
		if (file.Right(4) == _T(".avi") || file.Right(4) == _T(".mov") ||
			file.Right(4) == _T(".mkv") || file.Right(4) == _T(".wmv") || file.Right(4) == _T(".mp4"))
			return TRUE;
		else
			return FALSE;
	}
	...
	void CReadChangesRequest::ProcessNotification()
	{
		...
		for (;;)
		{
			...
			// Check if video file. If so, continue the process, or go to next file changes notification information
			// Action : FILE_ACTION_ADDED					1
			//			FILE_ACTION_REMOVED					2
			//			FILE_ACTION_MODIFIED				3
			//			FILE_ACTION_RENAMED_OLD_NAME		4
			//			FILE_ACTION_RENAMED_NEW_NAME		5
			if (IsVideoFile(wstrFilename) && (wstrFilename.Find(_T("RECYCLE.BIN"), 4) == -1) 
